...
RSpec.describe ClojureDsInRuby::PersistentVector do
  ...
  describe "#get" do
    let(:vector) { described_class[1, 2, 3, 4, 5] }
  ...
  describe "#set" do
    let(:vector) { described_class[1, 2, 3] }
  ...
  describe "#pop" do
    ...
    it "maintains immutability" do
      original = described_class[1, 2, 3]
  ...
  describe "#concat" do
    it "concatenates with another vector" do
      v1 = described_class[1, 2]
      v2 = described_class[3, 4]
      result = v1.concat(v2)
  ...
  describe "large vector operations" do
    it "handles vectors larger than branch factor" do
      large_vector = (1..100).reduce(described_class.empty) { |v, i| v.push(i) }
    ...
    it "maintains performance with deep trees" do
      large_vector = (1..1000).reduce(described_class.empty) { |v, i| v.push(i) }
  ...
  describe "structural sharing" do
    it "shares structure between vectors" do
      original = (1..50).reduce(described_class.empty) { |v, i| v.push(i) }
    ...
    it "maintains independence after structural sharing" do
      base = (1..10).reduce(described_class.empty) { |v, i| v.push(i) }
  ...
  describe "edge cases" do
    it "handles nil values" do
      vector = described_class[1, nil, 3]
      expect(vector.get(1)).to be_nil
      expect(vector.size).to eq(3)
    end

    it "handles mixed types" do
      vector = described_class[1, "hello", :symbol, [1, 2]]
      expect(vector.get(0)).to eq(1)
